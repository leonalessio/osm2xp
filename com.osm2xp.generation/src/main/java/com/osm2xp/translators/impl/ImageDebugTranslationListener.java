...
public class ImageDebugTranslationListener implements ITranslationListener {
	...
	protected Polygon calculateDrawPoly(OsmPolygon polygon) {
		...
		if (drawCenterX >= 0 && drawCenterX < IMGSIZE_X && drawCenterY >= 0 && drawCenterY < IMGSIZE_Y) {
			Shape awtShape = polygon.getPolygon().asAwtShape();
			Point2D[] pointArray = polygon.getPolygon().vertexArray();
	...
	protected Path2D calculateDrawPath(OsmPolyline poly) {
		Point2D[] pointArray = poly.getPolyline().vertexArray();
	...
	protected void checkSetBase(OsmPolyline poly) {
		if (baseX == Double.MIN_VALUE) {
			Box2D boundingBox = poly.getPolyline().boundingBox();
	...
	public void complete() {
		g2d.dispose();
		try {
			File inputFile = new File(GlobalOptionsProvider.getOptions()
					.getCurrentFilePath());
	...
	protected File getTestPngName(File inputFile) {
		int i = 0;
		File curFile;
		while((curFile = new File(inputFile.getParentFile(), GlobalOptionsProvider.getSceneName() + i + ".png")).exists()) {
			i++;
		}
		return curFile;
	}
