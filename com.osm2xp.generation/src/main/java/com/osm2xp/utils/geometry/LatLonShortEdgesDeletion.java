 ...
 *
 */
public class LatLonShortEdgesDeletion {
	...
	private static PolyEdgeDeletionRes deleteEdge(Polygon p, PolyEdge seg) {
		...
		Polygon p_ = Osm2XPGeometryFactory.getInstance().createPolygon(shell, holes);
	...
	private static RingEdgeDeletionOut deleteEdge(LineString lr, LineEdge seg){
		...
		if ( Math.abs(a) <= Math.PI/2+TOL_ANGLE && Math.abs(a) >= Math.PI/2-TOL_ANGLE ) {
			...
			Coordinate c = new LatLonCoordinate(p1.x+t*x1, p1.y+t*ya);

			Coordinate[] cs_ = new LatLonCoordinate[cs.length-1];
			...
			else return new RingEdgeDeletionOut(Osm2XPGeometryFactory.getInstance().createLinearRing(cs_), true);
		}
		...
		//parallel case 1
			...
			Coordinate c1 = new LatLonCoordinate(xMid+t1*dx, yMid+t1*dy);
			Coordinate c2 = new LatLonCoordinate(xMid+t2*dx, yMid+t2*dy);

			Coordinate[] cs_ = new LatLonCoordinate[cs.length-2];
			cs_[0] = c1; cs_[1] = c2;
			if (seg.id != 0) {
		...
		//parallel case 2
			...
			Coordinate c1_ = new LatLonCoordinate( p3.x + t1*(p2.x-p3.x) , p3.y + t1*(p2.y-p3.y) );
			Coordinate c2_ = new LatLonCoordinate( p0.x + t2*(p1.x-p0.x) , p0.y + t2*(p1.y-p0.y) );
